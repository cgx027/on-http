...
function httpServiceFactory(
    ...
    HttpService.prototype.start = function() {
        ...
        // setup proxy to on-static server
        var onStaticConfig = configuration.get('on-static');

        if(onStaticConfig) {
            var onStaticAddr = onStaticConfig.northbondAddress;
            var onStaticPort = onStaticConfig.northbondPort;

            // assert.string(onStaticAddr, "southbondAddress") ;
            // assert.string(onStaticPort, "southbondPort") ;

            _.forEach(['/images', '/iso'], function(route){
                var onStaticProxy = {};
                onStaticProxy.localPath = "/api/2.0" + route;
                onStaticProxy.server = "http://" + onStaticAddr + ':' + onStaticPort;
                onStaticProxy.remotePath = route;
                createHttpProxy(self.app, onStaticProxy);
            });
        }
